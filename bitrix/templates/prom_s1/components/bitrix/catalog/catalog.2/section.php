...
function getNumEnding($number, $cases, $incNum = true)
{
...
$this->SetViewTarget("goodsCount");
$flt = ["IBLOCK_ID"=>$arResult["IBLOCK"]["ID"], "SECTION_ID"=>$arResult["SECTION"]["ID"], "INCLUDE_SUBSECTIONS" => "Y", "ACTIVE"=>"Y"];
if(isset($GLOBALS['arrSubdomainSectionId'])){
	$flt[]['IBLOCK_SECTION_ID'] = $GLOBALS['arrSubdomainSectionId'];
}
$res = \CIBlockElement::GetList([], $flt, [], false, ["ID", "NAME", "IBLOCK_ID"]);
	echo getNumEnding($res, array('товар', 'товара','товаров'));
$this->EndViewTarget();
...
<?
$IBLOCK_ID = 138;
$arFilterr = Array('IBLOCK_ID'=>$IBLOCK_ID, 'GLOBAL_ACTIVE'=>'Y', 'CODE'=>$GLOBALS['PAGE'][count($GLOBALS['PAGE'])-2], 'UF_BRAND');
$res = CIBlockSection::GetList(Array($by=>$order), $arFilterr, false, Array('UF_*'));
while ($arResultt = $res->GetNext())
{ ?>


<?php if (!empty($arResultt['~UF_DOPSV'])): ?>
<div class="catalog-wrapper intec-content intec-content-visible">
        <div class="catalog-wrapper-2 intec-content-wrapper">
<div class="catalog-description catalog-description-top intec-ui-markup-text">
<?php echo $arResultt['~UF_DOPSV']; ?>
</div>
</div>
</div>
<?php endif; ?>
...
//$arFilter = Array('IBLOCK_ID'=>138, 'GLOBAL_ACTIVE'=>'Y', 'CODE'=>$GLOBALS['PAGE'][count($GLOBALS['PAGE'])-2]);
$db_list = CIBlockSection::GetList(Array(), $arFilter, false, array('ID','NAME','UF_SHORTCONTENT'));
while($ar_result = $db_list->Fetch()) {
    $arCurSection = $ar_result;
}

if (isset($GLOBALS['arrSectionFilter'])) {
	$arSections['PARAMETERS']['FILTER_NAME'] = 'arrSectionFilter';
}
//pre($arSections);
?>
    <div class="catalog-wrapper intec-content intec-content-visible">
        <div class="catalog-wrapper-2 intec-content-wrapper">
            <div class="sectionShortcontent">
                <?=$arCurSection['UF_SHORTCONTENT'];?>
            </div>
            ...
            <?= Html::beginTag('div', [
                ...
                if (empty($_GET['PAGEN_1']))
                    $APPLICATION->ShowViewContent($sTemplateId.'-description-top');
                ?>
                <?php if ($bSeo) { ?>
