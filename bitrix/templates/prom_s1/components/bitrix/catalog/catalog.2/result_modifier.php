...
if (
	...
	if (isset($GLOBALS['arrSectionFilter'])) {
		$arFilter = ArrayHelper::merge($arFilter, $GLOBALS['arrSectionFilter']);
	}
	...
	if(isset($GLOBALS['SUBDOMAINS']['SUBDOMAIN']) && !empty($arSection['DESCRIPTION'])){
		$arSection['DESCRIPTION'] = preg_replace("|<!--nosubdomains-->.*<!--/nosubdomains-->|isU", '', $arSection['DESCRIPTION']);
	}
	if(isset($GLOBALS['arrSubdomainSectionId']) && !empty($arSection['UF_ASSOC_CATS'])){
		$arSection['UF_ASSOC_CATS'] = array_intersect($arSection['UF_ASSOC_CATS'], $GLOBALS['arrSubdomainSectionId']);
	}
...
if(isset($GLOBALS['SUBDOMAINS']['SUBDOMAIN']) && !empty($arSection['DESCRIPTION'])){
	$arParams['DETAIL_PRODUCTS_SIMILAR_SHOW'] = 'N';
}
