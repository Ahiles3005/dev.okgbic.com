...
$catView = false;
if(!empty($arResult['SECTION']['UF_VIEW_TYPE'])) {
    $catViewField = CUserFieldEnum::GetList(array(), array("ID" => $arResult['SECTION']['UF_VIEW_TYPE']));
    if($catViewAr = $catViewField->GetNext()) {
        $catView = $catViewAr["VALUE"];
    }
}
if(in_array($catView, ['text', 'list', 'tile'])) {
    foreach($arViews as $arKey => $arView) {
        if($arView['VALUE'] != $catView) {
            unset($arViews[$arKey]);
        }
    }
} else {
...
if ($arElements['SHOW'] || !empty($arElements['TEMPLATE'])) {
    ...
    $arElements['PARAMETERS'] = ArrayHelper::merge($arElements['PARAMETERS'], [
        ...
        'LAZY_FIRST' => !empty($arResult['SECTION']['UF_DOPSV'])
    ]);
}
