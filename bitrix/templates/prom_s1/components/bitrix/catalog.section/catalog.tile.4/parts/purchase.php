    ...
    <?php $fRender = function (&$arItem, $bOffer = false) use (&$arResult, &$arVisual, &$APPLICATION, &$component, &$sTemplateId, &$arParentValues) { ?>
        <?php if ($bOffer || $arItem['ACTION'] === 'buy') { ?>
            ...
            <?php if ($arItem['CAN_BUY'] && !$bOffer) { ?>
                ...
                <?= Html::endTag('div') ?>
            <?php } else { ?>
<div class="orderPriceBtn-wrap">
    <div class="catalog-element-buy-button catalog-element-buy-add intec-ui intec-ui-control-basket-button intec-cl-background intec-cl-background-light-hover orderPriceBtn" data-role="item.order">
        <span class="intec-ui-part-content">
            Узнать цену
        </span>
    </div>
