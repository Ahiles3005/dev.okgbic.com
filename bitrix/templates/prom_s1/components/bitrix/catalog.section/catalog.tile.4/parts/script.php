        ...
        $(function () {
            ...
            root.update = function () {
                ...
                items.each(function () {
					...
					item.price.dop = $('[data-role="price.dop"]', item);
                    ...
                    item.update = function () {
                        ...
                        if (price !== null) {
			<?php if($arVisual['PRICE']['RECALCULATION']){ ?>
                            ...
                            item.price.base.html(price.base.display);
                                ...
                                if(item.offers.isEmpty()) {
                                    if(price.special_price != null){
                                        item.price.discount.html('<span class="title">' +price.title + '</span>' + price.special_price.PRICE
                                            + ((['пог', 'пог. м'].indexOf(entity.quantity.measure) >= 0) ? ('/'+entity.quantity.measure) : '')
                                        );
                                    } else {
                                        item.price.discount.html('<span class="title">' +price.title + '</span>' + price.discount.display
                                            + ((['пог', 'пог. м'].indexOf(entity.quantity.measure) >= 0) ? ('/'+entity.quantity.measure) : '')
                                        );
                                    }
                                } else {
//                                    item.price.discount.html(price.discount.display
                                    item.price.discount.html(price.min_offers_price_print
                                        + ((['пог', 'пог. м'].indexOf(entity.quantity.measure) >= 0) ? ('/'+entity.quantity.measure) : '')
                                    );
                                }
								...
								if (price.dop_price !== null) {
									...
									item.price.dop.html(dopPrice);
								}
							
                        } else {
                            item.price.attr('data-discount', 'false');
                            item.price.base.html(null);
                            item.price.discount.html('<?= getMessage('C_CATALOG_ELEMENT_DEFAULT_5_TEMPLATE_EMPTY_PRICE')?>');
							
							item.price.dop.html(null);
                        }
