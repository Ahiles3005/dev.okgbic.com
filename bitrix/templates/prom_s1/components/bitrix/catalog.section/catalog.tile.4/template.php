...
$arVisual['LAZY_FIRST'] = $arParams['LAZY_FIRST'];
...
<?= Html::beginTag('div', [
    ...
    <?= Html::beginTag('div', [
        ...
        <?php foreach ($arResult['ITEMS'] as $arItem_num => $arItem) {
            $arItem['num'] = $arItem_num;
            ...
            <?= Html::beginTag('div', [
                ...
                <div class="catalog-section-item-wrapper" data-borders-style="<?= $arVisual['BORDERS']['STYLE'] ?>">
                    <div class="catalog-section-item-base">
                        ...
                        <?php if (is_array($arItem['DISPLAY_PROPERTIES'])) { ?>
                            <div class="catalog-section-item-properties">
                                <div class="intec-grid intec-grid-o-vertical intec-grid-wrap intec-grid-i-h-8 intec-grid-i-v-2">
                                    <?php $view_prop_count = 0;
                                        foreach($arItem['DISPLAY_PROPERTIES'] as $p_v){
                                            if(in_array($p_v['NAME'], ['Артикул', 'ГОСТ, серия, альбом'])){
                                                continue;
                                            }
                                    ?>
                                            <?= Html::beginTag('div', [
                                                'class' => 'catalog-section-item-property-wrap intec-grid-item-auto',
                                            ]) ?>
                                                <div class="catalog-section-item-property">
                                                    <?= Html::tag('span', $p_v['NAME'], []) ?>
                                                    <?= Html::tag('span', $p_v['VALUE'], []) ?>
                                                </div>
                                            <?= Html::endTag('div') ?>
                                    <?php 
                                            if(++$view_prop_count >= 3){
                                                break;
                                            }
                                        } ?>
                                </div>
