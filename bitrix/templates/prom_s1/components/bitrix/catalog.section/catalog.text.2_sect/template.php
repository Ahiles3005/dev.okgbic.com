...
<? $YourID = $arItem["ID"]; $res = CIBlockElement::GetByID($YourID);if($arRes = $res->Fetch()){   $res = CIBlockSection::GetByID($arItem["~IBLOCK_SECTION_ID"]);   if($arRes = $res->Fetch())   {     $section_code = $arRes["CODE"]; 
  }}?>
<?//echo "<pre>"; print_r($arItem);
$arItem["DETAIL_PAGE_URL"] = "/catalog/".$section_code."/".$arItem["CODE"]."/";
?>
        <?php
...
//pre(json_decode($sData, 1));
        ...
        <div class="catalog-section-item-background">
            <div class="catalog-section-item-wrapper">
                <a href="<?=$arItem['DETAIL_PAGE_URL'];?>" class="fixedLink"></a>
