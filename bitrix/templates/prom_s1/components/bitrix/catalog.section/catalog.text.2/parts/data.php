    ...
    $fHandle = function (&$arItem, $bOffer = false) use (&$arResult, &$arVisual) {
        ...
        if (!empty($arItem['ITEM_PRICES']))
            ...
            foreach ($arItem['ITEM_PRICES'] as &$arPrice) {
				$infoPrice = CCatalogGroup::GetByID($arPrice['PRICE_TYPE_ID']);
                $arData['prices'][] = [
                    'title' => !empty($infoPrice['NAME']) ? $infoPrice['NAME'] : '',
					'quantity' => [
...
//                    'currency' => Loader::includeModule('currency')
                    'currency' => (!empty($arResult['VISUAL']['PRICE']['RECALCULATION']) && Loader::includeModule('currency')) 
                        ? CCurrencyLang::GetFormatDescription($arPrice['CURRENCY']) : null,
                    'dop_price' => $arItem['DOP_PRICE']['PRICE'] != '' ? $arItem['DOP_PRICE'] : null,
                    'special_price' => $arItem['SPECIAL_PRICE']['PRICE'] != '' ? $arItem['SPECIAL_PRICE'] : null,
                    'min_offers_price' => !empty($arItem['MIN_OFFERS_PRICE']) ? $arItem['MIN_OFFERS_PRICE'] : null,
                    'min_offers_price_print' => !empty($arItem['MIN_OFFERS_PRICE_PRINT']) ? $arItem['MIN_OFFERS_PRICE_PRINT'] : null,
                ];
