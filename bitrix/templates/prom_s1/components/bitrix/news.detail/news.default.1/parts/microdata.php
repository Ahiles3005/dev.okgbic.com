...
<div itemscope itemtype="http://schema.org/<?= $arVisual['MICRODATA']['TYPE'] ?>">
    <meta itemprop="headline name" content="<?= htmlspecialchars(
	$arResult['NAME'], 
    ...
    <?php if (!empty($arResult['PREVIEW_TEXT'])) { ?>
        <meta itemprop="description" content="<?= htmlspecialchars(
		strip_tags($arResult['PREVIEW_TEXT']),
		ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML5
	) ?>" />
    <?php } ?>
    <?php if (!empty($arResult['DETAIL_TEXT'])) { ?>
        <meta itemprop="articleBody" content="<?= htmlspecialchars(
