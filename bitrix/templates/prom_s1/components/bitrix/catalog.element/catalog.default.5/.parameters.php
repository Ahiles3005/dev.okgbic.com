...
if (!empty($arCurrentValues['IBLOCK_ID'])) {
	...
	$hPropertyHlBlock = function ($key, $arValue) {
        if ($arValue['PROPERTY_TYPE'] === 'S' && $arValue['USER_TYPE'] == 'directory' && $arValue['MULTIPLE'] === 'N')
            return [
                'key' => $arValue['CODE'],
                'value' => '['.$arValue['CODE'].'] '.$arValue['NAME']
            ];
    ...
    $arPropertyHl = $arProperties->asArray($hPropertyHlBlock);
		
//print_r($arPropertyHl);
	...
	$arTemplateParameters['PROPERTY_SIMILAR'] = [
        'PARENT' => 'DATA_SOURCE',
        'NAME' => Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_PROPERTY_SIMILAR'),
        'TYPE' => 'LIST',
        'VALUES' => $arPropertyLinkedMultiple,
        'ADDITIONAL_VALUES' => 'Y',
        'REFRESH' => 'Y'
...
if (!empty($arCurrentValues['PROPERTY_SIMILAR'])) {
    $arTemplateParameters['PRODUCTS_SIMILAR_SHOW'] = [
        'PARENT' => 'VISUAL',
        'NAME' => Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_PRODUCTS_SIMILAR_SHOW'),
        'TYPE' => 'CHECKBOX',
        'DEFAULT' => 'N',
        'REFRESH' => 'Y'
    ];

    if ($arCurrentValues['PRODUCTS_SIMILAR_SHOW'] === 'Y') {
        $arTemplateParameters['PRODUCTS_SIMILAR_NAME'] = [
            'PARENT' => 'VISUAL',
            'NAME' => Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_PRODUCTS_SIMILAR_NAME'),
            'TYPE' => 'STRING',
            'DEFAULT' => Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_PRODUCTS_SIMILAR_NAME_DEFAULT')
        ];

        if ($arCurrentValues['MAIN_VIEW'] !== '2') {
            $arTemplateParameters['PRODUCTS_SIMILAR_POSITION'] = [
                'PARENT' => 'VISUAL',
                'NAME' => Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_PRODUCTS_SIMILAR_POSITION'),
                'TYPE' => 'LIST',
                'VALUES' => [
                    'content' => Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_PRODUCTS_SIMILAR_POSITION_CONTENT'),
                    'column' => Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_PRODUCTS_SIMILAR_POSITION_COLUMN')
                ],
                'DEFAULT' => 'content',
                'REFRESH' => 'Y'
            ];

            if ($arCurrentValues['PRODUCTS_SIMILAR_POSITION'] === 'column') {
                $arTemplateParameters['PRODUCTS_SIMILAR_START_COUNT'] = [
                    'PARENT' => 'VISUAL',
                    'NAME' => Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_PRODUCTS_SIMILAR_START_COUNT'),
                    'TYPE' => 'LIST',
                    'VALUES' => [
                        0 => 'All',
                        3 => '3',
        ...
        include(__DIR__ . '/parameters/products.similar.php');
    }
}

$arTemplateParameters['CATEGORIES_ASSOCIATED_SHOW'] = [
    'PARENT' => 'VISUAL',
    'NAME' => Loc::getMessage('C_CATALOG_ELEMENT_DEFAULT_5_CATEGORIES_ASSOCIATED_SHOW'),
    'TYPE' => 'CHECKBOX',
    'DEFAULT' => 'N',
    'REFRESH' => 'Y'
];
