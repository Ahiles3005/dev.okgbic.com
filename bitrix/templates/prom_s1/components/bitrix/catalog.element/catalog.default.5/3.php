...
$arParams = ArrayHelper::merge([
    ...
    'PRODUCTS_SIMILAR_SHOW' => 'N',
    'PRODUCTS_RECOMMENDED_NAME' => null,
    'PRODUCTS_SIMILAR_NAME' => null,
    'PRODUCTS_RECOMMENDED_POSITION' => 'content',
    'PRODUCTS_SIMILAR_POSITION' => 'content',
    'PRODUCTS_SIMILAR_START_COUNT' => 0,
    'CATEGORIES_ASSOCIATED_SHOW' => 'N',
...
$arVisual = [
    ...
    'SIMILAR' => [
        'SHOW' => $arParams['PRODUCTS_SIMILAR_SHOW'] === 'Y',
        'NAME' => $arParams['PRODUCTS_SIMILAR_NAME'],
        'POSITION' => ArrayHelper::fromRange(['content', 'column'], $arParams['PRODUCTS_SIMILAR_POSITION']),
        'START_COUNT' => $arParams['PRODUCTS_SIMILAR_START_COUNT']
    ],
    'CATEGORIES_ASSOCIATED' => [
        'SHOW' => $arParams['CATEGORIES_ASSOCIATED_SHOW'] === 'Y',
    ],
    'ASSOCIATED' => [
...
if ($arVisual['MAIN_VIEW'] === '2') {
    ...
    $arVisual['SIMILAR']['POSITION'] = 'content';
...
$sectionRules = [
    1681 => [ 
        'NAGRUZKA_ATTR_L2' => '7bf24d534c0fade585f920bee843a2f9',
    ]
];

$sectionId = $arResult['SECTION']['ID'] ?? 0;


if (isset($sectionRules[$sectionId])) {
    

    // Получаем подходящие ТП
   $filter = [
    'PROPERTY_NAGRUZKA_ATTR_L2_XML_ID' => '7bf24d534c0fade585f920bee843a2f9'
];

$dbRes = CCatalogSKU::getOffersList(
   $arResult['ID'],
    0,
    $filter,
    ['ID', 'NAME'],
 ...
 $offers = $dbRes[$arResult['ID']] ?? [];

    if (!empty($offers)) {
     //   $firstOffer = reset($offers);
  $arResult['OFFERS_SELECTED']=51676;
   //     $arResult['OFFERS_SELECTED'] = $firstOffer['ID'];
$cp = $this->__component;
$cp->SetResultCacheKeys(['OFFERS_SELECTED']);
$cp->SetResultCacheKeys(['OFFERS']);
  //      echo(        $arResult['OFFERS_SELECTED']);
        // Подменяем данные товара на выбранное ТП
     //   $arResult['MIN_PRICE'] = CPrice::GetBasePrice($firstOffer['ID']);
     //   $arResult['OFFERS'] = [$firstOffer];
