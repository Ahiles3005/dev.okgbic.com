...
//        'show' => !empty($arPrice) ? 'true' : 'false',
    ...
    <div class="catalog-element-price">
        <div class="catalog-element-price-current catalog-element-price-part">
            <span class="catalog-element-price-current-value" <?if(!empty($arPrice)):?> data-role="price.discount"<?endif;?>>
                <?if(!empty($arPrice)):?>
                    <span class="title">&nbsp;</span>
                    <?= !empty($arPrice) ? $arPrice['PRINT_PRICE'] : null ?>
                <?else:?>
                   <?= getMessage('C_CATALOG_ELEMENT_DEFAULT_5_TEMPLATE_EMPTY_PRICE')?>
                <?endif;?>
            </span>
		
            <?if(!empty($arPrice)):?>
                <span class="catalog-element-price-current-separator"><?= (
                    (!empty($arResult['CATALOG_MEASURE_NAME']) && in_array($arResult['CATALOG_MEASURE_NAME'], ['пог', 'пог. м']))? '/': ''
                )?></span>
