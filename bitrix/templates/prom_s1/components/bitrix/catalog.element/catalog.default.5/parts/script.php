    ...
    (function ($, api) {
        $(function () {
			...
			if (['пог', 'пог. м'].indexOf(entity.quantity.measure) >= 0) {
                        ...
                        dynamic.price.discount.html("<?= getMessage('C_CATALOG_ELEMENT_DEFAULT_5_TEMPLATE_EMPTY_PRICE')?>");
...
//                        if (!offerCurrent || offerCurrent.sort > offer.sort)
            ...
            <?php if (!empty($arResult['OFFERS_SELECTED'])) { ?>
...
dynamic.properties.each(function () {
                        ...
                        let property = self.data('property');
                          console.log(property);
                                dynamic.offers.setCurrentByValue(property,<?= $arResult['OFFERS_SELECTED']?>);
