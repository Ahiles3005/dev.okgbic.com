...
if ( !empty($arSection['UF_SKU_SEL_VAL_ID'])  && !empty($arSection['UF_SKU_SEL_PROP']) ) {


    // Получаем подходящие ТП
    $filter = [
        'PROPERTY_'.$arSection['UF_SKU_SEL_PROP'].'_ID' => $arSection['UF_SKU_SEL_VAL_ID']
    ];
    ...
    if (!empty($offers)) {
        ...
        $arResult['OFFERS_SELECTED']=intval($arSection['UF_SKU_SEL_VAL_ID']);

        $this->getComponent()->SetResultCacheKeys(['OFFERS_SELECTED']);
