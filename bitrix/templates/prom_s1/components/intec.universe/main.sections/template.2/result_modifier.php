...
if(isset($GLOBALS['arrSubdomainSectionId'])){
	foreach($arResult['SECTIONS'] as $tmpSectionKey => $tmpSection){
		if(in_array($tmpSectionKey, $GLOBALS['arrSubdomainSectionId'])) {
			foreach($tmpSection['SECTIONS'] as $tmpSectionKey2 => $tmpSection2){
				if(!in_array($tmpSectionKey2, $GLOBALS['arrSubdomainSectionId'])) {
					unset($arResult['SECTIONS'][$tmpSectionKey]['SECTIONS'][$tmpSectionKey2]);
				}
			}
		} else {
			unset($arResult['SECTIONS'][$tmpSectionKey]);
		}
	}
}